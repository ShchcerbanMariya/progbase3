...
class GenerateData
{
    ...
    public Question[] GenerateQuestions(string nameFile, int questionsNum, DateTime firstLim, DateTime secondLim)
    {
        if (File.Exists(nameFile))
        {
            ...
            List<string> parseQuestions = File.ReadAllText(nameFile).Split('\n').ToList();
            Question[] quest = new Question[questionsNum];
            Random rnd = new Random();
            for (int i = 0; i < questionsNum; i++)
            {
                int randQId = rnd.Next(0, parseQuestions.Count());
                string[] currentQuestion = parseQuestions[randQId].Split(',');
    ...
    public Answer[] GenerateAnswers(string nameFile, int answersNum, Question[] questions)
    {
        if (File.Exists(nameFile))
        {
